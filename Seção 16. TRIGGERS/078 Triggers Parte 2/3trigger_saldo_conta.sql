...
BEGIN 
    ...
    END 
    ELSE IF (SELECT Count(*) FROM saldo_conta WHERE  conta_c = @conta_c) = 0 AND @OPER = 'D' 
    BEGIN 
		INSERT INTO saldo_conta VALUES (@CONTA_C, @VALOR *- 1) 
    END ELSE IF (SELECT Count(*) FROM   saldo_conta WHERE  conta_c = @conta_c) = 0 AND @OPER = 'C' 
    BEGIN 
		INSERT INTO saldo_conta VALUES (@CONTA_C, @VALOR) 
    END 
    ELSE IF (SELECT Count(*) FROM   saldo_conta WHERE  conta_c = @conta_c) > 0 AND @OPER = 'C' 
    BEGIN 
		UPDATE saldo_conta SET    saldo = saldo + @VALOR WHERE  conta_c = @CONTA_C 
	END 
	ELSE IF (SELECT Count(*) FROM   saldo_conta WHERE  conta_c = @conta_c) > 0 AND @OPER = 'D' 
    BEGIN 
		UPDATE saldo_conta SET    saldo = saldo - @VALOR WHERE  conta_c = @CONTA_C 
